...
two_weel_20 <- filter(vehicles, drive == "2-Wheel Drive", hwy > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
car_worst_hwy <- two_weel_20 %>%
  filter(
    hwy == min(hwy, na.rm = TRUE)
  )
...
most_hwy_model <- function(year_choice, make_choice) {
  return(vehicles %>%
    filter(
      year_choice == year,
      make_choice == make,
